...
import {
  Box, AppBar, Toolbar, IconButton, Typography, TextField, InputAdornment,
  FormControl, InputLabel, Select, MenuItem, Avatar, Button, Badge, useTheme,
} from "@mui/material";
...
export default function HeaderMobile({
  title, search, setSearch, filtro, setFiltro, onMenuClick,
  ...
  return (
    <Box>
      <AppBar position="fixed" elevation={1} sx={{ backgroundColor: theme.palette.background.paper }}>
        <Toolbar>
          ...
          {/* Direita */}
          {selected ? (
            <Button color="inherit" onClick={handleSair} size="small">Sair</Button>
          ) : null}
      ...
      {!selected && (
        <Box px={2} py={2}>
          <TextField
            fullWidth size="small" placeholder="Pesquisar pelo nome"
            value={effectiveSearch} onChange={(e) => effectiveSetSearch(e.target.value)}
